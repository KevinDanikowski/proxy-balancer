...
class Balancer {
  ...
  async request(url, options, timeout = this.config.timeout / 1000, { retryCount = 0, timesThisIpRetried = 0, ipsTried = 1 } = {}) {
    ...
    try {
      ...
      const retryChoice = await this.config.retryFn({ error: err, retryCount, timesThisIpRetried, ipsTried, proxy: next }, {
        ...
        // optionally pass nextProxy in the case that all proxies are blocked (i.e. in test case)
        blockIP: (duration = this.config.limiter.blockDuration || 5 * 60 * 1000, nextProxy = next) => {
          // Note: next is unavailable if there is no next due to no available proxies (i.e. all proxies are blocked)
          return Boolean(this.ipLimiter && duration && nextProxy && this.formatProxy(nextProxy)) ? this.ipLimiter.block(this.formatProxy(nextProxy), duration / 1000) : Promise.resolve('No ip limiter config present.')
        }
      })
